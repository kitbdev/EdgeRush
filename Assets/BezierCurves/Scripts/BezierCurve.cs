...
#if UNITY_EDITOR
            UnityEditor.Undo.RecordObject(bezierPoint.transform, "FlipX");
#endif
            var t = bezierPoint.position;
            t.x *= -1;
            bezierPoint.position = t;
#if UNITY_EDITOR
            UnityEditor.Undo.RecordObject(bezierPoint, "FlipX");
#endif
            t = bezierPoint.handle1;
            t.x *= -1;
            bezierPoint.handle1 = t;
            if (bezierPoint.handleStyle != BezierPoint.HandleStyle.Connected) {
                t = bezierPoint.handle2;
                t.x *= -1;
                bezierPoint.handle2 = t;
            }
        }
    }
